<template>
    <Header />
    <h1 class="text-center mb-11 mt-10 text-3xl font-bold leading-none text-black dark:text-white">Ma collection</h1>

    <div class="grid grid-cols-1 gap-6 justify-center sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3">
        <div v-for="cour in couses" @click="" class="max-w-sm mb-7 bg-gray-100 border shadow-xl group hover:border-red-700 border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700">
            <a href="#">
...
import {getCours} from '@/api/user.js'
...
onMounted(()=>{
    getCours(user.id).then(res=>{
        couses.value=res;
        console.log(couses.value)
    })
})
